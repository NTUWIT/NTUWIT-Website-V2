 ...
 * on every request. Cache failures fall through to a live query rather than
 * failing the page.
 */
export async function readLeaderboard(): Promise<LeaderboardRow[]> {
  const cached = await redis.get<LeaderboardRow[]>(CACHE_KEY).catch(() => null);
  if (cached) return cached;

  const rows = await getLeaderboard();
  await redis.set(CACHE_KEY, rows, { ex: CACHE_SECONDS }).catch(() => {});
  return rows;
}
